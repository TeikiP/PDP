  ...
  if (input_file != NULL) {
    ...
    while (fscanf(input_file, "%s", line) != EOF) {
      ...
      if (line[i] == right_delimiter) {
        ...
        if (room_length == 0 || room_length > grid_size) {
          printf("Error in input file format.\n");
          free_grid(grid);
          return NULL;
        }
...
static grid_t* random_rooms(grid_t* grid) {
  /* Each room starts with a length of one cell and
   * has an (100 - percent_to_not_expand)% chance to grow by 1 cell.
   * Everytime it expands, the chance is reiterated until it fails
   * or until it hits the length limit. */
  const int percent_to_not_expand = 20;
      ...
      if (grid->cells[y * grid->size + x]->room_length == 0) {
        ...
        if (direction == 0) {
          ...
          do {
            ...
            random_percent = rand() % 100 + 1;
              ...
              length < MAX_ROOM_SIZE_GENERATOR && length < (grid->size - y) &&
              grid->cells[(y + length) * grid->size + x]->room_length == 0 &&
              random_percent > percent_to_not_expand);
          /* The conditions to exit the above loop are, in the order listed : to
           * reach the hard cap for room size, to reach the end of the grid, to
           * border a cell that already has a room, to fall short of the minimum
           * percent needed to keep expanding the room. */
        ...
        /* rightwards */
          ...
          do {
            length++;
            random_percent = rand() % 100 + 1;
          } while (length < MAX_ROOM_SIZE_GENERATOR &&
                   length < (grid->size - x) &&
